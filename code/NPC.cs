...
public sealed class NPC : Component, IGameEventHandler<PlayerDeath>, IGameEventHandler<DeathEvent>
{
	...
	{
		var nearestPlayer = Scene.GetAllComponents<PlayerController>().FirstOrDefault( x => x.WorldPosition.Distance( WorldPosition ) < distance );
	...
	public void MoveToPlayer()
	{
		...
		if ( nearestPlayer.IsValid() )
			MoveTo( nearestPlayer.WorldPosition );
	}

	public PlayerController NearestPlayer()
	{
		return Scene.GetAllComponents<PlayerController>().OrderBy( x => x.WorldPosition.Distance( WorldPosition ) ).FirstOrDefault();
	}
	...
	public bool AtDestination( float buffer = 50 )
	{
		...
		return Agent.WorldPosition.Distance( Destination ) < buffer;
	}
	...
	protected override void OnUpdate()
	{
		if ( Networking.IsHost )
		{
			...
			if ( IsMoving && Agent.WorldPosition.Distance( Destination ) < 250f && !Stop )
			{
				IsMoving = false;
				WishMove = false;
	...
	public void Attack( int damage = 10, float fireRate = 5, float length = 200 )
	{
		...
		var tr = Scene.Trace.Ray( WorldPosition, WorldPosition + WorldRotation.Forward * length )
			.WithoutTags( "zombie" )
